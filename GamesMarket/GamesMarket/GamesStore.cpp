...
void GamesStore::add_game(User user, std::string name, std::string desc, int price) {
...
void GamesStore::reg(std::string nickname, std::string login, std::string password) {
...
bool GamesStore::login(std::string login, std::string password) {
	for (int i = 0; i < _users.size(); i++) {
		if (_users[i].getUser_Login() == login && _users[i].getUser_Password() == password) {
			return true;
		}
	}
	return false;
